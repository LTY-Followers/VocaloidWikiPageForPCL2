        ...
        case "acgwiki/pcl2":
            ...
            with open(random.choice(pagelist),"r") as f:
                if "Mozilla" in request.user_agent:
                    ...
                    response.status = 301
                    response.headers["location"] = "https://github.com/wuliaodexiaoluo/ACGWikiPageForPCL2"
                    response.content_type = "text/html"
                    response.headers["server"] = "ACGWikiPageHTTPServer/0.1.0(Based on Python Flask)"
                if not request.url_root in bind_url:
                    with open("./html/403.html") as hf:
                        response = make_response(hf.read().replace("${reason}","使用的 Url 不在请求白名单中").replace("${solve}","请前往<a href='https://github.com/wuliaodexiaoluo/ACGWikiPageForPCL2'>项目地址</a>获取正确的地址").encode("utf-8"))
                        response.status = 403
